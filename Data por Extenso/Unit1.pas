...
begin
        ...
        begin
                ...
                qtd2 := qtd2 + 1;
                Form1.qtd[StrToInt(Form1.sql2['numero_mes'])] := Form1.qtd[StrToInt(Form1.sql2['numero_mes'])] + 1;
...
for i:=0 to 11 do
begin
        if (Form1.vetor_mes[i] <> 0) or (Form1.qtd[i] <> 0) then
        begin
                porc := Form1.vetor_mes[i] / Form1.qtd[i];
                Form1.lstporcentagem.Items.Add((floattostrf(strtofloat(FloatToStr(porc)),ffFixed,2,2)));
        end
        else
        begin
              Form1.lstporcentagem.Items.Add('0,00');
        end;
end;
end;
...
begin
  ...
  if sql.Eof then
    begin
        ...
        botao := 1;
        lblmsg.Caption := 'Digite as  informações acima e clique em confirmar para cadastrar';
        cbomes.SetFocus;
    end
  else
    begin
        ShowMessage('Código já cadastrado digite outro código');
        voltar();
    end;
...
var
volume,soma_refugados,porc,vol  : double;
i : integer;
begin
...
begin
  ...
  if sql.Eof then
    begin
        ShowMessage('Código não cadastrado digite outro código');
        voltar();        
    end
  else
    begin
        ...
        lblmsg.Caption := 'Altere as informações desejadas e clique em Confirmar para Alterar o registro';
        cbomes.SetFocus;      
    end;
end;
